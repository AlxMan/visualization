...
@Service("iUserService")   // 不写括号中的内容也可，默认就是用接口注入
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<User> login(String username, String password) {
        ...
        //3.调用mapper的登录方法, 返回一个user对象 (如果返回的user==null，则说明密码错误)
    ...
    public ServerResponse<String> add(User user) {
        ...
        if (validResponse.getStatus() != ResponseCode.SUCCESS.getCode()) {  //不等说明用户名存在
    ...
    // 有效性检查的一个重载方法，来验证更新时的用户名和邮箱是否已经存在，但可以是该用户现有的用户名和邮箱-----------------------
    public ServerResponse<String> checkValid(User user, String type) {

        if (StringUtils.isNotBlank(type)) {
            //校验用户名
            if (Const.USERNAME.equals(type)) {
                //查询用户名是否存在
                int resultCount = userMapper.checkUpdateUserName(user);
                if (resultCount > 0) {
                    return ServerResponse.createByErrorMsg("用户名已经存在");
                }
            }
            ...
            //校验邮箱
            if (Const.EMAIL.equals(type)) {
                int resultCount = userMapper.checkUpdateEmail(user);
                if (resultCount > 0) {
                    return ServerResponse.createByErrorMsg("Email已经存在");
                }
            }
    ...
    public ServerResponse<String> update(User user) {
        ...
        //1.校验用户名是否存在
        ServerResponse<String> validResponse = this.checkValid(user, Const.USERNAME);
        if (validResponse.getStatus() != ResponseCode.SUCCESS.getCode()) {
            return validResponse;
        }

        //2.校验邮箱是否已经存在
        validResponse = this.checkValid(user, Const.EMAIL);
        if (validResponse.getStatus() != ResponseCode.SUCCESS.getCode()) {
            return validResponse;
        }
        ...
        //4.调用mapper 中的方法进行更新 ,得到返回值 count==1 更新成功， count==0 ,更新失败
